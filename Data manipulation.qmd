...
# Load the data
file_path = "Data/exportAN10v2.dsv"
output_path = "Data/exportAN10v3.dsv"

try:
    # Assuming the file is tab-separated (.dsv often uses tabs as delimiter)
    data = pd.read_csv(file_path, sep="|")
    
    # Ensure the column 'PERIODO_ID' exists
    if 'PERIODO_ID' in data.columns:
        # Filter the data where PERIODO_ID is greater than 20120101
        filtered_data = data[data['PERIODO_ID'] > 20120101]
        
        # Save the filtered data back to a .dsv file
        filtered_data.to_csv(output_path, sep="|", index=False)
        print(f"Filtered data saved to '{output_path}'.")
    else:
        print("The column 'PERIODO_ID' was not found in the dataset.")
...
file_paths = {
    "AN3": "Data/exportAN3.dsv",
    "AN10": "Data/exportAN10v2.dsv",
    "ENTIDAD": "Data/exportENTIDAD.dsv",
    "REP4B2": "Data/exportREP4B2.dsv",
    "REP4B3": "Data/exportREP4B3.dsv",
    "UBIGEO": "Data/exportUBIGEO.dsv"
}
...
merged_AN3.to_csv("Data/an03_final.dsv", sep='|', index=False)
...
#  Save the result to a .dsv file
merged_AN10.to_csv("Data/an10_final.dsv", sep='|', index=False)
...
an10_aggregated.to_csv("Data/an10_aggregated.dsv", sep='|', index=False)
...
an10_aggregated2.to_csv("Data/an10_aggregated2.dsv", sep='|', index=False)
...
merged_REP4B2.to_csv('Data/rep4b2_final.dsv', sep='|', index=False)
...
AN3 = pd.read_csv("Data/an03_final.dsv", delimiter='|', encoding='latin1')
...
AN10 = pd.read_csv("Data/an10_final.dsv", delimiter='|', encoding='latin1')
...
lines = alt.Chart(monthly_data).mark_line(color='orange').encode(
    x=alt.X('YearMonth:T'),
    y=alt.Y('Annual_Growth_Rate:Q', title='Annual Growth Rate (%)', scale=alt.Scale(domain=[y_min, y_max]), axis=alt.Axis(titleColor='orange'))
).interactive()
